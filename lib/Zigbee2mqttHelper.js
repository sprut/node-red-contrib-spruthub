...
class SprutHubHelper {
        ...
        if ("brightness" in payload) {
            if ("state" in payload && payload.state == "OFF") {
                ...
                if ("color" in payload  && "hue" in payload.color && "saturation" in payload.color) {
                    ...
                    var rgb = SprutHubHelper.cie2rgb(payload.color.x, payload.color.y, payload.brightness);
                    var hsv = SprutHubHelper.rgb2hsv(rgb.r, rgb.g, rgb.b);
                    hue = hsv.h;
                    sat = hsv.s;
                }
                var bri = SprutHubHelper.convertRange(parseInt(payload.brightness), [0, 255], [0, 100]);
                var ct = "color_temp" in payload?SprutHubHelper.convertRange(parseInt(payload.color_temp), [50,400], [140,500]):null;
    ...
    static formatPayload(payload, device) {
        ...
        if ("color" in payload && "x" in payload.color) {
            ...
            var rgb = SprutHubHelper.cie2rgb(payload.color.x, payload.color.y, bri);
            var hsv = SprutHubHelper.rgb2hsv(rgb.r, rgb.g, rgb.b);
            result['color'] = {
                "rgb":rgb,
                "hsv":hsv
