            ...
            $.getJSON('spruthub/getDevices', {
                ...
                .done(function (data, textStatus, jqXHR) {
                    try {
                        ...
                        //groups
                        groupHtml = $('<optgroup/>', {label: RED._("node-red-contrib-spruthub/in:multiselect.groups")});
                        ...
                        //devices
                        groupHtml = $('<optgroup/>', {label: RED._("node-red-contrib-spruthub/in:multiselect.devices")});
                        groupHtml.appendTo(selectedItemElement);
...
function z2m_getItemStateList(nodeItem, selectedItemElementName, options = {}) {
    ...
    function z2m_updateItemStateList(controller, selectedItemElement, itemName) {
        ...
        if (controller && uniqueId) {
            $.getJSON('spruthub/getLastStateById', {
                controllerID: controller.id,
                device_id:uniqueId
            })
                .done(function (data, textStatus, jqXHR) {
                    try {
                        selectedItemElement.html('<option value="0">'+ RED._("node-red-contrib-spruthub/in:multiselect.complete_payload")+'</option>');

                        var groupHtml = '';

                        if (data[0] && Object.keys(data[0]).length) {
                            groupHtml = $('<optgroup/>', {label: RED._("node-red-contrib-spruthub/in:multiselect.spruthub")});
                            groupHtml.appendTo(selectedItemElement);
                        ...
                        if (data[1] && Object.keys(data[1]).length) {
                            ...
                            groupHtml = $('<optgroup/>', {label: RED._("node-red-contrib-spruthub/in:multiselect.homekit")});
                            groupHtml.appendTo(selectedItemElement);
