...
module.exports = function(RED) {
    class SprutHubNodeBridge {
        constructor(config) {
            ...
            //         fill: "yellow",
            //         shape: "ring",
            //         text: "node-red-contrib-spruthub/bridge:status.searching"
            //     });
            //     //
            ...
            if (node.server) {
                ...
                node.status({
                    fill: "red",
                    shape: "dot",
                    text: "node-red-contrib-spruthub/bridge:status.no_server"
                });
            }
            ...
            //                 message_in.selector = SprutHubHelper.generateSelector(message_in.topic);
            ...
            //                 }
            //             } else {
            //                 var data_array = SprutHubHelper.prepareDataArray(node.server, channels);
            ...
            //                     fill: "red",
            //                     shape: "dot",
            //                     text: "node-red-contrib-spruthub/bridge:status.no_value"
            //                 });
            //             }
            //             node.cleanTimer = setTimeout(function () {
            //                 node.status({}); //clean
            ...
            //                 fill: "red",
            //                 shape: "dot",
            //                 text: "node-red-contrib-spruthub/bridge:status.no_device"
            //             });
            //         }
            //     });
            //
        ...
        onMQTTMessageBridge(data) {
            ...
            if (node.server.getBaseTopic()+'/bridge/state' == data.topic) {
                ...
                if (SprutHubHelper.isJson(data.payload)) {
                    ...
                    if ("type" in parsedData) {
                        if ("device_connected" == parsedData.type) {
                            node.status({
                                fill: "green",
                                shape: "ring",
                                text: "node-red-contrib-spruthub/bridge:status.connected"
                            });
                            node.cleanTimer = setTimeout(function(){
                                node.setNodeStatus();
                            }, 3000);
                            ...
                            if ("interview_started" == parsedData.message) {
                                node.status({
                                    fill: "yellow",
                                    shape: "ring",
                                    text: "node-red-contrib-spruthub/bridge:status.pairing"
                                });
                                node.cleanTimer = setTimeout(function(){
                                    node.setNodeStatus();
                                }, 10000);
                                ...
                                node.status({
                                    fill: "green",
                                    shape: "dot",
                                    text: "node-red-contrib-spruthub/bridge:status.paired"
                                });
                                node.cleanTimer = setTimeout(function(){
                                    node.setNodeStatus();
                                }, 3000);
                                ...
                                node.status({
                                    fill: "red",
                                    shape: "dot",
                                    text: "node-red-contrib-spruthub/bridge:status.failed"
                                });
                                node.cleanTimer = setTimeout(function(){
                                    node.setNodeStatus();
                                }, 3000);
    ...
    RED.nodes.registerType('spruthub-bridge', SprutHubNodeBridge);
};
